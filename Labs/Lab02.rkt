...
(define (head-temp head)
  (... (head-mouth head) ... (head-radius) ...))

; worm-temp: Worm -> ?
(define (worm-temp worm)
  (... (worm-its-name worm) ... (worm-head worm) ... (worm-body worm)))
...
(define BACKGROUND (above (rectangle BG-WD HT3 "solid" "red")
                   (above (rectangle BG-WD HT2 "solid" "yellow") (rectangle BG-WD HT1 "solid" "green"))))
...
(check-expect (place-terrain TERRAIN-EASY) (place-image CLIMBER (/ BG-WD 2) TERRAIN-EASY BACKGROUND))
(check-expect (place-terrain TERRAIN-MEDIUM) (place-image CLIMBER (/ BG-WD 2) TERRAIN-MEDIUM BACKGROUND))
(check-expect (place-terrain TERRAIN-HARD) (place-image CLIMBER (/ BG-WD 2) TERRAIN-HARD BACKGROUND))

; climb: Terrain KeyEvent -> Terrain
; Moves the climber up the mountain if the keyevent is correct

(define (climb terrain keyEvent)
  (cond [(string=? keyEvent "up") (- terrain 3)]
        [else (+ terrain 5)]))

(check-expect (climb TERRAIN-EASY "up") (- TERRAIN-EASY 3))
(check-expect (climb TERRAIN-EASY "down") (+ TERRAIN-EASY 5))
...
; Creates the climber and the mountain
